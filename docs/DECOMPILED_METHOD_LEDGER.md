...
| Shop inventory construction                                                                          | Builds the shop item array directly from the current event payload: the three-byte header is skipped and every remaining byte is an item ID into `k.a`. No hero-level or per-item-level check occurs in this construction path. | Confirmed | `javap -c -p j`; allocates `k[]` with `event.e.length - 3` and copies `k.a[event.e[3 + i] & 0xff]`. |
...
| Equipment rune slots              | Confirmed `k.g` is the slot count. The equipment UI loops from `0` to `k.g - 1` to render rune positions. Rune attachment applies only to category-3 weapons and category-2 subtype-1 main armor; main armor decodes it from the tail low nibble, weapons from `byte_f` bits `7..5`. |
...
| Shop equipment progression       | Confirmed not stored in `item.dat` as a universal five-level requirement. Shop stock is supplied by individual script/event payloads, so location/story routing determines which listed items can be purchased. |

...
final class GameDatStatusPatcher {
  ...
  static PatchSummary patch(byte[] data, List<StatusPatch> patches) {
    ...
    for (StatusPatch patch : patches) {
      ...
      if (o.durationOffset >= 0) {
        data[o.durationOffset] = checkedByte(patch.duration(), "duration");
        summary.duration++;
      } else {
        summary.skipped++;
      }
      if (o.expireOffset >= 0) {
        data[o.expireOffset] = encodeSignedChance(patch.expireChance());
        summary.expire++;
      } else {
        summary.skipped++;
      }
      if (o.iconOffset >= 0) {
        data[o.iconOffset] = checkedByte(patch.icon(), "icon");
        summary.icon++;
      } else {
        summary.skipped++;
      }
  ...
  static int getPacked(byte[] data, int n, boolean specialFlag) {
  ...
  static int getN(int n, int flags) {
    if ((flags & 0x10) != 0) {
      n++;
    }
    if ((flags & 8) != 0) {

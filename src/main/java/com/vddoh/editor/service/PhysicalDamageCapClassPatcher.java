...
public final class PhysicalDamageCapClassPatcher {
  ...
  private static final int FLAG_MASK = -1024;
  ...
  public static State state(byte[] data) {
    try {
      ...
      log.info(
          "Physical damage cap class patch state unknown; originalSites={}, patchedSites={}",
          original,
          patched);
        ...
        classFile.transformClass(
            ...
            java.lang.classfile.ClassTransform.transformingMethodBodies(
                ...
                java.lang.classfile.CodeTransform.ofStateful(
                    () -> new PhysicalDamageCapCodeTransform(counter))));

    State patchedState = state(patched);
    if (counter.count() != EXPECTED_SITES || patchedState != State.PATCHED) {
      throw new IllegalStateException(
          "Physical damage cap patch did not produce the expected g.class bytecode; counter=%d, state=%s"
  ...
  private static boolean hasCapImmediatelyBefore(List<Instruction> instructions, int offset) {
    return offset >= 14
        && isHeroResultDamageRead(instructions, offset - 14)
  ...
  private static boolean isHeroResultDamageRead(List<Instruction> instructions, int offset) {
    return offset + 3 < instructions.size()
        ...
        && isPush(instructions.get(offset + 2), DAMAGE_MASK)
  ...
  private static final class PhysicalDamageCapCodeTransform
    ...
    private PhysicalDamageCapCodeTransform(PatchCounter counter) {

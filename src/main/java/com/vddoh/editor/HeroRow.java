...
final class HeroRow implements Serializable {
  ...
  HeroRow(
    ...
    this.speed = speed;
    this.levelCap = originalLevelCap = levelCap;
    this.baseCritChance = originalBaseCritChance = baseCritChance;
    this.baseCritDamage = originalBaseCritDamage = baseCritDamage;
    this.notes = notes;
  }
  ...
  StatCurve stat(int index) {
    return switch (index) {
      ...
      case 2 -> vitality;
      case 3 -> speed;
      default -> throw new IllegalArgumentException("Unknown hero stat %d".formatted(index));
    };
  }

...
public final class FxItemsView extends BorderPane {
  ...
  private TableView<FxItemViewModel> table() {
    ...
    table
        ...
        .addAll(
            ...
            intColumn("Price", "price", 80),
            intColumn("Icon", "icon", 72),
            intColumn("HP+", "hpBonus", 72),
            intColumn("Res+", "resourceBonus", 72),
  ...
  private void buildItemPatch(Button build) {
    List<com.vddoh.editor.ItemEdit> edits =
        items.stream().filter(FxItemViewModel::changed).map(FxItemViewModel::toEdit).toList();
    if (edits.isEmpty()) {
  ...
  private static Object propertyValue(FxItemViewModel item, String property) {
    return switch (property) {
      ...
      case "icon" -> item.icon();
      case "hpBonus" -> item.hpBonus();
      case "resourceBonus" -> item.resourceBonus();
  ...
  private static ObservableValue<Number> integerValue(FxItemViewModel item, String property) {
    return switch (property) {
      ...
      case "price" -> item.priceProperty();
      case "icon" -> item.iconProperty();
      case "hpBonus" -> new SimpleIntegerProperty(item.hpBonus());
      case "resourceBonus" -> new SimpleIntegerProperty(item.resourceBonus());
